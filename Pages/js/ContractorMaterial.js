...
$(function () {
    ...
    const data = abp.libs.datatables.normalizeConfiguration({
        ...
        columnDefs: [
            ...
            //                         $("#contractorId").val(data.record.contractorId);
            //                         $('#paymentFile').click();
        ...
        form.ajaxForm({
            ...
            success: function (data) {
                if (data === "Error") {
                    ...
                    let str = data.map(x => `${x.orderId}: Actual ${x.actual}, Upstream ${x.upstream}`).join("</br>");
                    promptHTML(str, `Mismatch Invoices: ${data.length} Items`);
                }
